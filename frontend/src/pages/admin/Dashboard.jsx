  ...
  return (
    <div className="bg-[#111113] border border-zinc-800 rounded-2xl p-6 flex items-center gap-5">
      ...
      <div>
        ...
        <p className="font-display font-semibold text-2xl text-zinc-100">
          {value ?? <span className="text-zinc-600 text-lg">—</span>}
        </p>
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#111113] border border-zinc-800 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-zinc-800 flex items-center justify-between">
            ...
            <a href="/admin/contacts" className="text-xs text-indigo-400 hover:text-indigo-300 transition-colors">View all</a>
          </div>
          {data?.latestContacts.length === 0 ? (
            <p className="px-5 py-8 text-sm text-zinc-600 text-center">No contacts yet.</p>
          ) : (
        ...
        <div className="bg-[#111113] border border-zinc-800 rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-zinc-800 flex items-center justify-between">
            ...
            <a href="/admin/newsletter" className="text-xs text-indigo-400 hover:text-indigo-300 transition-colors">View all</a>
          </div>
          {data?.latestSubscribers.length === 0 ? (
            <p className="px-5 py-8 text-sm text-zinc-600 text-center">No subscribers yet.</p>
          ) : (
            <ul className="divide-y divide-zinc-800/60">
