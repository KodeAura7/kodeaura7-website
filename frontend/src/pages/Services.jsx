    ...
    <SiteLayout>
      ...
      <SectionHero eyebrow="What We Do" title="Services Designed to" gradient="Scale Your Business" description="Four specialised disciplines. One unified vision. Engineered for growth.">
        <div className="fade-up delay-3 flex flex-wrap items-center justify-center gap-3">
          {services.map((svc) => (
            <a key={svc.id || svc.slug} href={`#${svc.slug || svc.id}`}
      ...
      {services.map((service, index) => {
        ...
        const activeFeatures = (service.features || []).filter((f) => (typeof f === 'string' ? true : f.enabled !== false));
        const anchor = service.slug || service.id;
        const ctaLabel = service.cta_label || `Start a ${service.name} Project`;
          ...
          <section key={service.id || service.slug} id={anchor} className="py-20 md:py-28 border-t border-zinc-900 scroll-mt-24">
            <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
              <div className={reverse ? 'lg:order-2' : 'lg:order-1'}>
                        ...
                        return (
                          <li key={label} className="flex items-center gap-3 text-sm text-zinc-300">
                            <span className="w-5 h-5 rounded-full flex items-center justify-center shrink-0"
                              style={{ background: `color-mix(in srgb,${service.accent} 12%,transparent)`, border: `1px solid color-mix(in srgb,${service.accent} 22%,transparent)`, color: service.accent }}>
                ...
                <Link to="/#contact" className="inline-flex items-center gap-2 text-sm font-medium mt-8 transition-opacity hover:opacity-80" style={{ color: service.accent }}>
                  {ctaLabel} <Icon icon="solar:arrow-right-linear" width={16} />
                </Link>
              </div>
